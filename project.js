...
//补充函数
function list_min(xs, compare) {
    if (is_null(xs)) {
        return null;
    }
    function helper(lst, current_min) {
        return is_null(lst)
            ? current_min
            : compare(head(lst), current_min)
              ? helper(tail(lst), head(lst))
              : helper(tail(lst), current_min);
    }
...
function update_player(gameObject) {
    const base_speed = 4;
    const moveSpeed = boosted ? base_speed * 2 : base_speed;
    const jumpPower = 8.5;
    ...
    gui_label("Score: " + stringify(10 * math_floor(score)), 40,550);  // 左上角显示
    gui_label("use W/A/D to move" , 40,580); 
    gui_label("Watch out the DURIAN!!!" , 40,610); 
    gui_label("&" , 90,640); 
    gui_label("Make use of the SPEED red Nonya Wares", 40,670); 
...
const url = get_brick_url_from_gameobject(other);
  ...
  if (!is_null(url)) {
    ...
    if (url === list_ref(wall_images, 1)) {
      ...
      set_update(cam, x => {
  gui_label("Final Score: " + stringify(10 * math_floor(score)), 1000, 1000);
});
...
let all_waters = list(water); // 初始只有第一张水图
...
function update_water(gameObject) {
    ...
    // 水上升逻辑
    if (water_start) {
        water_speed = water_speed + 0.0004;

        // 所有水图一起上升
        all_waters = map(w => {
            translate_world(w, vector3(0, delta_time() * water_speed, 0));
            return w;
        }, all_waters);
    }

    // 生成新水图逻辑（只在最底部水体升高后触发）
    const lowest_water = list_min(all_waters, (a, b) =>
        get_y(get_position(a)) < get_y(get_position(b))
    );
    const lowest_y = get_y(get_position(lowest_water));
    const ground_threshold = 0; // 砖块底部基准

    if (lowest_y > ground_threshold) {
        const new_water = instantiate_sprite("https://raw.githubusercontent.com/wxy0429/NUS-SICP/refs/heads/main/wave1.png");
        remove_collider_components(new_water);
        const new_y = lowest_y - 11; // 与上一张水图无缝拼接
        set_position(new_water, vector3(0, new_y, -100));
        remove_collider_components(new_water);
        set_scale(new_water, vector3(1.6, 1.8, 1));
        all_waters = pair(new_water, all_waters);
    }
